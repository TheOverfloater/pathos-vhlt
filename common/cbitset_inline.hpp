...
//=============================================
inline void CBitSet::set( unsigned int index )
{
...
//=============================================
inline void CBitSet::reset( unsigned int index )
{
	assert(index < m_numBits);
	m_pDataArray[index/NB_BITS_IN_BYTE] &= ~(1U << (index%NB_BITS_IN_BYTE));
}
...
//=============================================
inline void CBitSet::flip( unsigned int index )
{
	assert(index < m_numBits);
	m_pDataArray[index/NB_BITS_IN_BYTE] ^= (1U << (index%NB_BITS_IN_BYTE));
}
...
//=============================================
inline bool CBitSet::test( unsigned int index ) const
{
	assert(index < m_numBits);
...
inline bool CBitSet::all( void ) const
{
	...
	unsigned int numFullBytes = m_numBits / NB_BITS_IN_BYTE;
	...
	unsigned int i = numFullBytes * NB_BITS_IN_BYTE;
	for(; i < m_numBits; i++)
	{
		if(!test(i))
			return false;
...
//=============================================
inline void CBitSet::resize( unsigned int size )
{
	// Remember these for porting values over
	unsigned int originalBitCount = m_numBits;
	...
	if(pOriginalArray)
	{
		unsigned int refBitCount = (originalBitCount < m_numBits) ? originalBitCount : m_numBits;
		unsigned int numFullBytes = refBitCount / NB_BITS_IN_BYTE;
		for(unsigned int i = 0; i < numFullBytes; i++)
...
//=============================================
inline unsigned int CBitSet::count( void ) const
{
...
//=============================================
inline unsigned int CBitSet::size( void ) const
{
	return m_numBits;
}
...
//=============================================
inline unsigned int CBitSet::numbytes( void ) const
{
	return m_numBytes;
}
...
inline CBitSet& CBitSet::operator |= ( const CBitSet& other )
{
	unsigned int srcSize = other.size();
	unsigned int myOriginalSize = size();
	if(srcSize > myOriginalSize)
		resize(srcSize);

	// Copy only the smaller scope
	const byte* pOtherBytes = other.getDataConst();
	unsigned int mergeSize = (myOriginalSize > srcSize) ? srcSize : myOriginalSize;
	unsigned int mergeCopyBytes = (mergeSize / NB_BITS_IN_BYTE);
	unsigned int mergeFullByteCount = ceil(static_cast<float>(mergeSize) / static_cast<float>(NB_BITS_IN_BYTE));

	// Directly merge with the full byte range
	for(unsigned int i = 0; i < mergeCopyBytes; i++)
	...
	{
		unsigned int i = mergeCopyBytes * NB_BITS_IN_BYTE;
		for(; i < mergeSize; i++)
		{
...
inline CBitSet& CBitSet::operator &= ( const CBitSet& other )
{
	...
	if(mergeFullByteCount > mergeCopyBytes)
	{
		...
		unsigned int totalMergeBitCount = mergeFullByteCount * NB_BITS_IN_BYTE;
	...
	{
		for(unsigned int i = mergeFullByteCount; i < m_numBytes; i++)
			m_pDataArray[i] = 0x00;
	}
...
inline CBitSet CBitSet::operator ~ ( void ) const
{
	...
	unsigned int wholeBytesCount = (resultSet.m_numBits / NB_BITS_IN_BYTE);

	for(unsigned int i = 0; i < wholeBytesCount; i++)
		resultSet.m_pDataArray[i] = ~resultSet.m_pDataArray[i];

	unsigned int i = wholeBytesCount*NB_BITS_IN_BYTE;
...
inline bool CBitSet::operator == ( const CBitSet& other )
{
	...
	unsigned int wholeBytesCount = (m_numBits / NB_BITS_IN_BYTE);
	for(unsigned int i = 0; i < wholeBytesCount; i++)
	{
