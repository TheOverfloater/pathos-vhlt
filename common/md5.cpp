...
//
//=============================================
CMD5::CMD5( const byte* pbuffer, unsigned int bufsize )
{
	Init();
...
//
//=============================================
void CMD5::Decode( unsigned int *poutput, const byte *pinput, unsigned int length )
{
	for(unsigned int i = 0, j = 0; j < length; i++, j += 4)
		poutput[i] = (static_cast<unsigned int>(pinput[j])) 
		| (static_cast<unsigned int>(pinput[j+1]) << 8)
		| (static_cast<unsigned int>(pinput[j+2]) << 16)
		| (static_cast<unsigned int>(pinput[j+3]) << 24);
}
...
//
//=============================================
void CMD5::Encode( byte *poutput, const unsigned int* pinput, unsigned int length )
{
	for(unsigned int i = 0, j = 0; j < length; i++, j += 4) 
	{
...
//
//=============================================
void CMD5::Update( const byte *pinput, unsigned int length )
{
	// compute number of bytes mod 64
	unsigned int index = m_count[0] / 8 % MD5_BLOCKSIZE;
	...
	const unsigned int firstpart = 64 - index;
	unsigned int i;
...
CMD5& CMD5::Finalize()
{
	...
	if (!m_isFinalized) 
	{
		...
		const unsigned int index = m_count[0] / 8 % 64;
...
//
//=============================================
std::string CMD5::HexDigest( void ) const
{
	if(!m_isFinalized)
		return "";
