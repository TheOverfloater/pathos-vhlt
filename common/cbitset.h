...
public:
	...
	static const unsigned int NB_BITS_IN_BYTE = 8*sizeof(byte);

public:
	CBitSet( void );
	explicit CBitSet( unsigned int size );
	CBitSet( const CBitSet& src );
	CBitSet( const byte* pdataarray, unsigned int numbits );
	CBitSet( unsigned int bitsetSize, const unsigned int inputBits[], unsigned int arraySize );
	~CBitSet( void );
...
public:
	...
	inline unsigned int numbytes( void ) const;
