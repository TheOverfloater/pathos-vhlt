...
void LoadStudioModel(const char* modelname, const vec3_t origin, const vec3_t angles, const vec3_t scale, int body, int skin, int trace_mode, entity_t* ent, bool vlight)
{
...
void LoadStudioModels( void )
{
	...
	for( int i = 0; i < g_numentities; i++ )
	{
		...
		if( !Q_stricmp( name, "env_model" ))
		{
			...
			if( !model || !*model )
			{
				Developer( DEVELOPER_LEVEL_WARNING, "env_model has empty model field\n" );
				continue;
			}
		}
		...
		bool vlight = (IntForKey(e, "zhlt_vertexlight") == 1);
		LoadStudioModel(model, origin, angles, xform, body, skin, trace_mode, e, vlight);
	}
